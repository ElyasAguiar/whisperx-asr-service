...
def recognize_file(
    ...
    logger.info(
        f"Config: model={model}, language={language}, diarization={enable_diarization}"
    )
        ...
        for i, result in enumerate(response.results):
            ...
            for j, alternative in enumerate(result.alternatives):
                ...
                if alternative.words:
                    ...
                    logger.info("  " + "-" * 76)
                    logger.info(
                        f"  {'Word':<20} {'Start':<10} {'End':<10} {'Confidence':<12} {'Speaker':<10}"
                    )
                    logger.info("  " + "-" * 76)

                    for word in alternative.words[:20]:  # Show first 20 words
                        speaker = (
                            f"SPEAKER_{word.speaker_tag:02d}"
                    ...
                    if len(alternative.words) > 20:
                        logger.info(
                            f"  ... and {len(alternative.words) - 20} more words"
                        )
...
def main():
    parser = argparse.ArgumentParser(description="gRPC client for WhisperX ASR Service")
    ...
    parser.add_argument(
        "--model", default="large-v3", help="Model name (default: large-v3)"
    )
    parser.add_argument(
    ...
    parser.add_argument("--streaming", action="store_true", help="Use streaming API")
