...
class AsrServiceServicer(asr_pb2_grpc.AsrServiceServicer):
    ...
    def _config_from_proto(self, config: asr_pb2.RecognitionConfig) -> ASRConfig:
        ...
        return ASRConfig(
            ...
            num_speakers=(
                config.diarization_speaker_count
                if config.diarization_speaker_count > 0
                else None
            ),
            min_speakers=(
                config.min_speaker_count if config.min_speaker_count > 0 else None
            ),
            max_speakers=(
                config.max_speaker_count if config.max_speaker_count > 0 else None
            ),
            initial_prompt=config.initial_prompt if config.initial_prompt else None,
            sample_rate=(
                config.sample_rate_hertz if config.sample_rate_hertz > 0 else 16000
            ),
    ...
    def Recognize(
        self, request: asr_pb2.RecognizeRequest, context
    ) -> asr_pb2.RecognizeResponse:
        """
        ...
        try:
            ...
            if request.audio.content:
                ...
                context.abort(
                    grpc.StatusCode.INVALID_ARGUMENT, "No audio content provided"
                )
                return
            ...
            audio_bytes = self._convert_audio_to_wav(
                ...
                (
                    ...
                    if request.config.sample_rate_hertz > 0
                    else 16000
                ),
                config.audio_encoding,
                (
                    request.config.audio_channel_count
                    if request.config.audio_channel_count > 0
                    else 1
                ),
            )
                ...
                for word_info in segment.words:
                    ...
                    if word_info.speaker:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to parse speaker tag '{word_info.speaker}': {e}"
                            )
    ...
    def StreamingRecognize(
        self, request_iterator: Iterator[asr_pb2.StreamingRecognizeRequest], context
        ...
        try:
            ...
            for request in request_iterator:
                if request.HasField("streaming_config"):
                    ...
                    logger.info(
                        f"Streaming config received: model={config.model}, language={config.language}"
                    )
            ...
            if not config:
                context.abort(
                    grpc.StatusCode.INVALID_ARGUMENT, "No streaming config provided"
                )
                return

            if not audio_chunks:
                context.abort(
                    grpc.StatusCode.INVALID_ARGUMENT, "No audio content received"
                )
                return
            ...
            logger.info(
                f"Received {len(audio_chunks)} audio chunks, total {len(audio_bytes)} bytes"
            )
            ...
            context.abort(
                grpc.StatusCode.INTERNAL, f"Streaming recognition failed: {str(e)}"
            )
...
def serve_grpc(
    ...
    asr_pb2_grpc.add_AsrServiceServicer_to_server(
        AsrServiceServicer(asr_engine), server
    )
