...
class WhisperXASR(ASRInterface):
    ...
    def __init__(
        ...
        logger.info(
            f"WhisperX ASR initialized - device: {device}, compute_type: {compute_type}"
        )
    ...
    def transcribe(self, audio_data: bytes, config: ASRConfig) -> ASRResult:
        ...
        try:
            ...
            if config.enable_diarization and self.hf_token:
                logger.info(
                    f"Starting speaker diarization with model: {config.diarization_model}"
                )
                try:
                    ...
                    if hasattr(diarize_segments, "exclusive_speaker_diarization"):
                        diarize_segments = (
                            diarize_segments.exclusive_speaker_diarization
                        )
            ...
            duration = (
                audio.shape[0] / config.sample_rate if len(audio.shape) > 0 else 0.0
            )
